...
- **Component *uses* are not Components.** Cloak and Slide are two uses of the **Ghost** Component; Zap and Defend are uses of **Cracker**; Breach and Encrypt/Decrypt are uses of **Codebreak**; and so on. Only the names in `config.mjs` `COMPONENT_LABELS` exist as Components — a use has no rank, no training, and no `system.components.<slug>` entry. Only the Netrunning Components have uses; the authoritative list is `COMPONENT_USES` in `config.mjs`.
  - **AE targets:** a Component-wide effect uses `system.components.<component>.bonus`; an effect on a single action uses `system.componentUses.<use>.bonus` (e.g. Skunk's −2 hits `cloak` and `slide` but leaves the rest of Ghost alone). Use slugs are flat and unique across all Components. `system.components.cloak.bonus` is *not* a path and fails silently.
